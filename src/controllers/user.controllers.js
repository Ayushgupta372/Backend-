...
export const registerUser = asyncHandler(async (req, res) => {
  console.log("1. Request received");
    ...
    return res.status(201).json(
        new ApiResponse(201, createdUser, "User registered Successfully") // Changed 200 to 201 for Created
    );
});
...
const user=  await User.findOne({
  $or:[{username},{email}] // mongodb opperater
 })
...
const {accessToken, refreshToken} = await generateAccessAndRefereshTokens(user._id)
 ...
 export const logoutUser = asyncHandler(async(req, res) => {
  ...
  //GET USEER details from frontend 
  //validation - not empty 
  //check if user alreay exists :username ,email
  //check for img ,check for avater 
